...
st.set_page_config(page_title="ADF Test with Macro‑Determined Breaks", layout="wide")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'adf_breaks_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Mean‑reversion score: {etf['adf_score_norm']:.3f}</p>
